...
A run pins an immutable pipeline revision: ID/version, stage keys, typed input/output schema versions, allowed predecessors, default model-profile references, review requirements and configuration hash. Baseline ships one registered definition. Stages are existing code handlers; configuration cannot introduce executable code, arbitrary URLs or new capabilities. Validate graph acyclicity, schema compatibility, reachable exit, stage count (maximum 20), and required approval barriers. Policy, authorization, evidence validation, audit, budget reservation and provider-egress enforcement are mandatory non-skippable controls. Optional registered stages may be skipped only if none of those controls, downstream required inputs or approval barriers is removed. New handler kinds require a code change and review.
