...
2. Data lead defines scoped schema/grants and control-function contracts; one person owns migration ordering. Prove queue runtime privilege requirements before accepting role grants. **In progress on `feat/m1-db-schema`:** reviewed SQL migrations in `packages/db/sql`, role-separated schemas, RLS and control dispatch functions, pg-boss 12.30.0 privilege verification, PostgreSQL integration tests.
