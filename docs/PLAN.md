...
3. Identity lead wires bootstrap/session/capabilities to a single API boundary; shared tests cover pool/caching and denied storage access. **In progress on `feat/m1-identity`:** Better Auth sessions, offline owner bootstrap, `@dce/policy` capability matrix, `@dce/storage` scoped local adapter, `/api/v1` authorize-and-scope boundary, pool-scope integration tests.
