...
2. Data lead defines scoped schema/grants and control-function contracts; one person owns migration ordering. Prove queue runtime privilege requirements before accepting role grants. **Done in PR #3 (merged `c1b8fc6`):** reviewed SQL migrations in `packages/db/sql`, role-separated schemas, RLS, control dispatch functions with actor membership validation, pg-boss 12.30.0 privilege verification, PostgreSQL integration tests in CI (`postgres-integration` job), migrator-path verification without admin grant repairs, runtime roles denied write access to `app.schema_migration`.
3. Identity lead wires bootstrap/session/capabilities to a single API boundary; shared tests cover pool/caching and denied storage access. **Done in PR #4 (merged `b5a36b9`, owner sign-off 6 September 2026):** Better Auth sessions, offline owner bootstrap, named capability grants, `@dce/policy` capability matrix, `@dce/storage` scoped local adapter, `/api/v1` authorize-and-scope boundary, identity-table privilege tests, pool-scope and client-context integration tests, root `.env` loading, and gitleaks default-rule regression coverage. HTTP route E2E and browser isolation tests remain follow-ups for task 4/first authenticated page.
4. Interface lead applies design tokens, translation keys and local fonts; worker lead adds fixture-only dispatcher skeleton with no provider keys. **In progress on `feat/m1-task4`:** `@dce/i18n` (en/th keys), `@dce/ui` (tokens, AppShell, Button), self-hosted Space Grotesk + Noto Sans Thai via `@fontsource/*`, login page + authenticated `/app` shell, browser isolation header test, `apps/worker` fixture dispatcher loop, migration `0009_mark_dispatch_delivered.sql`.
