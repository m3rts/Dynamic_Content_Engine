...
1. Foundation lead lands CI and the merge-protection setup first, alongside the minimal pinned workspace needed to execute meaningful checks. Record exact stable dependency versions/compatibility, package manager/runtime pins and dependency approvals. Do not add application features ahead of this gate. **In progress:** pinned pnpm/TypeScript workspace (`packages/contracts`, `packages/domain`, `packages/policy`, `packages/observability`), deterministic CI workflow (`.github/workflows/ci.yml`), local `pnpm run check` evidence on branch `feat/m1-foundation`. Branch protection unavailable on private free-tier repo — owner manual no-merge-until-checks procedure required until plan upgrade or repo visibility change.
...
If account plan cannot enforce branch protection, owner must document that platform limitation and use a manual no-merge-until-checks procedure; no claim of protected main. CI/branch settings are not activated by this document. **Verified 6 September 2026:** `m3rts/Dynamic_Content_Engine` is private on a GitHub plan that returns HTTP 403 for branch protection APIs; required checks cannot be enforced on `main` until the owner upgrades or makes the repository public. GitHub Advanced Security dependency review is also unavailable on this private plan; CI uses `pnpm audit --audit-level=high` instead.
