...
2. Data lead defines scoped schema/grants and control-function contracts; one person owns migration ordering. Prove queue runtime privilege requirements before accepting role grants. **In progress on `feat/m1-db-schema` (PR #3):** reviewed SQL migrations in `packages/db/sql`, role-separated schemas, RLS, control dispatch functions, pg-boss 12.30.0 privilege verification, PostgreSQL integration tests in CI (`postgres-integration` job). **Review fixes pending merge:** migrator-path CI/test (no admin grant repairs), `app.schema_migration` restricted from runtime roles, dispatch insertion validates active owner/operator membership.
